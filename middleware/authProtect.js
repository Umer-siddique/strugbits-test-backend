...
exports.authProtect = catchAsync(async (req, res, next) => {
  ...
  if (!verifiedUser) {
    return next(
      new ErrorHandler("User no longer available. Please login again")
    );
  }
  ...
  const isPasswordChangedAfter = await verifiedUser.PasswordChangedAfter(
    decoded.iat
  );
...
exports.authProtectAdmin = catchAsync(async (req, res, next) => {
  let token;
  const { authorization } = req.headers;
  if (authorization && authorization.startsWith("B")) {
    token = authorization.split(" ")[1];
  ...
  if (verifiedUser.role !== "admin") {
    return next(
      new ErrorHandler("Only the Admin has the right to do this task.")
    );
  }
